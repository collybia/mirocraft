...
- Панель говорит, какой из адресов машины давать друзьям, и подписывает их по имени
  сетевого адаптера — Hamachi, Radmin, локальная сеть, служебный.
- Кнопка «открыть порт наружу»: панель просит роутер по UPnP.
- Если у провайдера общий адрес и роутер настраивать бесполезно, панель говорит об этом
  прямо — и предлагает релей: маленький форвардер на любой машине с белым адресом,
  который даёт друзьям обычный адрес и порт. Ставить им не нужно ничего.
