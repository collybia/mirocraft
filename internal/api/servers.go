...
func (a *API) handleCreateServer(w http.ResponseWriter, r *http.Request) {
	...
	server.Dir = filepath.Join(a.dataDir, "servers", server.ID)

	if err := os.MkdirAll(server.Dir, 0o750); err != nil {
	...
	eulaPath := filepath.Join(server.Dir, "eula.txt")
...
func (a *API) handleDeleteServer(w http.ResponseWriter, r *http.Request) {
	...
	if server.Dir != "" {
		if err := os.RemoveAll(server.Dir); err != nil {
			a.log.Warn("removing the server directory failed",
				slog.String("dir", server.Dir), slog.String("error", err.Error()))
		}
	}
...
func (a *API) metricsFor(ctx context.Context, server *store.Server, status string) *serverMetrics {
	...
	if pong, err := mcping.Ping(pingCtx, "127.0.0.1", server.Port); err == nil {
